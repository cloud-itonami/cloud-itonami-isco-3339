(ns business-services-agency.governor-test
...
(deftest escalates-on-above-threshold-brokerage
  (let [st (fresh-store)
        proposal {:op :broker-above-threshold :effect :propose :confidence 0.9 :stake :high}
        v (governor/check {:client-id "client-1"} {} proposal st)]
    (is (:escalate? v))
    (is (not (:hard? v)))))
...
(deftest store-records-and-ledger-append-only
  (let [st (fresh-store)]
    (store/commit-record! st {:client-id "client-1" :op :match})
