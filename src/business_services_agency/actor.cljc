(ns business-services-agency.actor
  "BusinessServicesAgencyActor — the ISCO-08 3339 independent
  business-services-agency actor as a `langgraph.graph/state-graph`
  (per ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  agency operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the AgencyAdvisor can never directly
  commit a record or dispatch a robot action the
  BusinessServicesGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [business-services-agency.advisor :as advisor]
            [business-services-agency.governor :as governor]
            [business-services-agency.store :as store]))

(defn build-graph
  "Build a compiled BusinessServicesAgencyActor graph. `store`
  implements `business-services-agency.store/Store`. `advisor`
  implements `business-services-agency.advisor/Advisor` (defaults to
